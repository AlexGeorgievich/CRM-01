...
- создавать, блокировать и активировать пользователей, сбрасывать пароль;
- создавать и архивировать статусы, курсы и источники;
- просматривать список заявок;
- фильтровать заявки по статусу, курсу, источнику, ответственному менеджеру и просрочкам;
- искать заявку по имени клиента, контакту, e-mail или заметке;
- создавать заявку;
...
- `PATCH /api/v1/dictionaries/statuses/{status_id}` — редактирование или архивация статуса
- `GET /api/v1/dictionaries/courses` — курсы
- `PATCH /api/v1/dictionaries/courses/{course_id}` — редактирование или архивация курса
- `GET /api/v1/dictionaries/sources` — источники
- `PATCH /api/v1/dictionaries/sources/{source_id}` — редактирование или архивация источника
- `GET /api/v1/reports/summary` — KPI и базовая аналитика
- `GET /api/v1/users` — пользователи, только администратор
- `PATCH /api/v1/users/{user_id}` — изменение роли, статуса, имени или email пользователя
- `POST /api/v1/users/{user_id}/reset-password` — сброс пароля пользователя
...
- Рабочая админ-панель пользователей и справочников.
- CSV export/import.
- Поле `next_contact_date` и фильтр просрочек.
- Поле `email` в заявке, поиск и CSV импорт/экспорт по нему.
